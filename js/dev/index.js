...
function initSliders() {
  if (document.querySelector(".swiper")) {
    const swiper = new Swiper(".swiper", {
      // <- Вказую тут const swiper (було new Swiper без присвоєння)
      ...
      // Ефекти
      effect: "fade",
      // ---- ДОБАВЛЕНО: используем функцию, чтобы autoplay был включён только при width <= 930 ----
    ...
    window.addEventListener("resize", () => {
      ...
      resizeTimeout = setTimeout(() => {
        const cfg = getAutoplayConfig();
        if (cfg) {
          if (!swiper.params.autoplay) {
            swiper.params.autoplay = cfg;
...
function initHeaderScroll() {
  ...
  if (!header) return;
  const scrollTrigger = parseInt(header.dataset.scroll, 10);
  const headerToggle = () => {
...
document.querySelectorAll("[data-scroll]").forEach((btn2) => {
